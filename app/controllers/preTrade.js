              ...
              if (tx.data.startsWith(methodSignature)) {
                ...
                if (currentUser) {
                  ...
                  await Promise.all([
                    (async () => {
                      try {
                        const newBlock = await waitForBlockIncrement(
                          provider,
                          blockNumber
                        );
...
const waitForBlockIncrement = async (provider, currentBlockNumber) => {
  let newBlockNumber;

  // Poll every second until the block number increases
  do {
    await new Promise((resolve) => setTimeout(resolve, 1000)); // 1 second delay
    newBlockNumber = await provider.getBlockNumber();
    console.log("Waiting for next block... Current block:", newBlockNumber);
  } while (newBlockNumber <= currentBlockNumber);

  console.log("New block detected:", newBlockNumber);
